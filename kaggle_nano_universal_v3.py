...
# ===== PATHS =====
ROOT         = Path("/kaggle/temp/nano_universal")
HF_CACHE     = ROOT / "hf_cache"
MODEL_CACHE  = ROOT / "source_hf"
# Nome dell'artefatto si adatta al modello (es. final_artifact_Qwen2.5-14B-Instruct)
ARTIFACT_NAME= f"final_artifact_{MODEL_ID.split('/')[-1]}"
ARTIFACT_DIR = Path(f"/kaggle/working/{ARTIFACT_NAME}")
...
def discover_targets(model, suffixes, layer_start, layer_end):
    out = []
...
class TrueQuantLinear(nn.Module):
    def __init__(self, prot_q, prot_scale, prot_idx, deg_q, deg_scale, deg_idx, out_features, bias=None, bits=8, device="cuda:0"):
        super().__init__()
        self.out_features, self.bits = out_features, int(bits)
        self.register_buffer("prot_q", prot_q.to(device=device, dtype=torch.int8))
        self.register_buffer("prot_scale", prot_scale.to(device=device, dtype=torch.float16))
        self.register_buffer("prot_idx", prot_idx.to(device=device, dtype=torch.long))
        self.register_buffer("deg_q", deg_q.to(device=device, dtype=torch.int8))
        self.register_buffer("deg_scale", deg_scale.to(device=device, dtype=torch.float16))
        self.register_buffer("deg_idx", deg_idx.to(device=device, dtype=torch.long))
        
        if bias is not None: self.register_buffer("bias", bias.to(device=device, dtype=torch.float16))
        else: self.bias = None
...
def _load_tensor(key, device="cuda"):
    shard = MODEL_CACHE / tensor_to_file[key]
    with safe_open(str(shard), framework="pt", device=device) as f: return f.get_tensor(key)
def load_weight(mn, device):
    key = f"{mn}.weight"
    if key not in _w_cache: _w_cache[key] = _load_tensor(key, device).to(torch.float16).contiguous()
    return _w_cache[key]
def load_bias(mn, device):
    key = f"{mn}.bias"
    return _load_tensor(key, device).to(torch.float16).contiguous() if key in tensor_to_file else None
def row_importance(mn, device):
    if mn not in _r_cache:
        w = load_weight(mn, device) 
        _r_cache[mn] = torch.argsort(w.float().pow(2).mean(1).sqrt(), descending=False)
    return _r_cache[mn]
def clear_cache(mn):
    _w_cache.pop(f"{mn}.weight", None); _r_cache.pop(mn, None)

def build_candidate(model, mn, bits, prot_rows):
    dev = next(get_module(model, mn).parameters()).device
    w, bias, order = load_weight(mn, str(dev)), load_bias(mn, str(dev)), row_importance(mn, str(dev))
    total = w.shape[0]
...
print(f"[{MODEL_ID}] Detected {num_layers} hidden layers.")
...
class TrueQuantLinear(nn.Module):
    def __init__(s, pq, ps, pi, dq, ds, di, of, bias=None, bits=8, device="cuda:0"):
        super().__init__(); s.out_features, s.bits = of, int(bits)
        s.register_buffer("pq", pq.to(device=device, dtype=torch.int8)); s.register_buffer("ps", ps.to(device=device, dtype=torch.float16))
        s.register_buffer("pi", pi.to(device=device, dtype=torch.long)); s.register_buffer("dq", dq.to(device=device, dtype=torch.int8))
        s.register_buffer("ds", ds.to(device=device, dtype=torch.float16)); s.register_buffer("di", di.to(device=device, dtype=torch.long))
        if bias is not None: s.register_buffer("bias", bias.to(device=device, dtype=torch.float16))
        else: s.bias = None
    def forward(s, x):
        d, dt = x.device, x.dtype; f = x.to(torch.float16).reshape(-1, x.shape[-1])
...
def load_artifact(ad):
    ...
    for n, s in st.items():
        dev = next(get_module(m, n).parameters()).device
        b = s["bits"]
        ...
        _set(m, n, TrueQuantLinear(
            ...
            s["out_features"], s.get("bias"), b, device=str(dev)))
    return m.eval(), t, sp
'''
...
print(f"ZIP File generated: {ARTIFACT_DIR}.zip")
